...
static int create_proc_file(struct proc_file *file, struct proc_dir_entry *parent, const char* entry_name, int size, struct seq_operations *seq_ops, struct file_operations *fops);

#define add_proc_file_worker(name)						\
									\
int name##_counter = 1;                \
...
#define add_remove_proc_file_worker(name)    \
      \
int remove_proc_file_##name(struct proc_file *file)    \
{     \
	int ret = 0;      \
      \
	if (file != NULL) {     \
...
#define add_write_proc_buffer_worker(name)      \
      \
int write_proc_buffer_##name(struct proc_file *file, const char *buffer, unsigned int count)     \
{     \
	int ret = 0;      \
	int len;    \
	int len1, len2;      \
...
#define add_proc_file(name)            add_proc_file_worker(name)
#define add_remove_proc_file(name)     add_remove_proc_file_worker(name)
#define add_write_proc_buffer(name)    add_write_proc_buffer_worker(name)
...
static int proc_buffer_init(struct proc_file *file, int size)
{
...
static int proc_buffer_free(struct proc_file *file)
{
	kfree(file->buffer);
	file->buffer = NULL;
...
add_remove_proc_file(DRIVER_NAME)
add_write_proc_buffer(DRIVER_NAME)
