...
{
   int err;
   /* {SW} TODO: if needed we can inform the cp430 that we are gonna power down */
   ...
   err = twl_i2c_write_u8(TWL4030_MODULE_PM_MASTER, STARTON_PWON,
                          TWL4030_PM_MASTER_CFG_P1_TRANSITION);
   if (err)
      pr_err("TWL4030 Unable to setup STARTON_PWON in CFG_P1_TRANSITION reg\n");

   err = twl_i2c_write_u8(TWL4030_MODULE_PM_MASTER, STARTON_PWON,
                          TWL4030_PM_MASTER_CFG_P2_TRANSITION);
   if (err)
      pr_err("TWL4030 Unable to setup STARTON_PWON in CFG_P2_TRANSITION reg\n");

   err = twl_i2c_write_u8(TWL4030_MODULE_PM_MASTER, STARTON_PWON,
                          TWL4030_PM_MASTER_CFG_P3_TRANSITION);
   if (err)
      pr_err("TWL4030 Unable to setup STARTON_PWON in CFG_P3_TRANSITION reg\n");
