...
#define CAPTURE_OPERATION 1
#define PREVIEW_OPERATION 0
...
static bool operation = PREVIEW_OPERATION;
...
static int ov5640_old_capture(struct v4l2_subdev *subdev)
{
	...
	if (ret) {
		printk("ov5640_new_reg_list failed\n");
		goto out;
	}
	printk("ov5640_new_reg_list written\n");
	
	printk("Writing AEC/AGC manual\n");
	ret = ov5640_write_reg(client, 0x3503, 0x07);		//Manual AEC/AGC
	if (ret) { goto out; }
	
	printk("Reading preview register values\n");
	...
	printk("Preview Exposure %d, Gain %d, Maxlines %d\n",prevExp,prevGain,prevMaxlines);
	...
	printk("ov5640_5m_cap written\n");
	
	/*tmpreg = ov5640_read_reg(client, 0x350c);    		//Maxlines
	capMaxlines = (((uint) tmpreg) << 8);
	tmpreg = ov5640_read_reg(client, 0x350d);
	capMaxlines += tmpreg;*/
	printk("Capture Maxlines %d\n",capMaxlines);
	...
	ret = ov5640_write_reg(client, 0x350b, tmpreg);	
	if (ret) { goto out; }
	printk("Capture register values written\n");
...
static int ov5640_af_def_config(struct v4l2_subdev *subdev)
{
	...
	if (ret) {
		printk("DSG: %s() write ov5640_af_reg failed\n", __func__);
		v4l_err(client, "Failed to load af firmware\n");
	}
	else
	{
		printk("DSG: %s() write ov5640_af_reg completed\n", __func__);
		// wait until the status is idle
		while (ov5640_read_reg(client, 0x3029)!=0x70) mdelay(1);

		// set ov5640 to continuous mode
		ret = ov5640_write_reg(client, 0x3022, 0x04);
		if (ret) {
			printk("DSG: %s() failed to set to continuous mode\n", __func__);
...
static int ov5640_s_stream(struct v4l2_subdev *subdev, int streaming)
{
	...
	if (streaming) {
		...
		case V4L2_MBUS_FMT_UYVY8_1X16:
			printk("DSG: Format code is V4L2_MBUS_FMT_UYVY8_1X16\n");
			fmtreg = 0x32;
			fmtmuxreg = 0;		//DSG: Format ISP YUV 422
			break;
		case V4L2_MBUS_FMT_YUYV8_1X16:
			printk("DSG: Format code is V4L2_MBUS_FMT_YUYV8_1X16\n");
			fmtreg = 0x30;
			fmtmuxreg = 0;		//DSG: Format ISP YUV 422
			break;
		case V4L2_MBUS_FMT_YUYV8_2X8:
			printk("DSG: Format code is V4L2_MBUS_FMT_YUYV8_2X8\n");
			fmtreg = 0x30;
			fmtmuxreg = 0;
			break;
		case V4L2_MBUS_FMT_UYVY8_2X8:
			printk("DSG: Format code is V4L2_MBUS_FMT_UYVY8_2X8\n");
			fmtreg = 0x32;
			fmtmuxreg = 0;
			break;
		default:
			/* This shouldn't happen */
			printk("DSG: Format code is unknown %08x\n", (u32)ov5640->format.code);
			ret = -EINVAL;
			return ret;
		}
		...
		if (ret){
			printk("DSG: ov5640_config_timing failed\n");
			return ret;
		}
		...
		if (ret) {
			printk("DSG: clear power down bit failed\n");
			goto out;
		}
		printk("%s - New Stream started\n", __FUNCTION__);
				  ...
				  struct v4l2_subdev_format *fmt)
{
	...
	if((ov5640->format.width == OV5640_CAPTURE_WIDTH)  && (ov5640->format.height == OV5640_CAPTURE_HEIGHT)){
		if(defConfDone && (operation == PREVIEW_OPERATION)){
			printk("Configuring for capture\n");
		...
		if(defConfDone && (operation == CAPTURE_OPERATION)){
			printk("Configuring for streaming\n");
			ov5640_conf_back_vga(subdev);
			operation = PREVIEW_OPERATION;
		}
	}else{
		printk("Configuring is not correct\n");
	}
...
static int ov5640_s_ctrl(struct v4l2_ctrl *ctrl)
{
	...
	case V4L2_CID_BRIGHTNESS:
		printk("DSG: %s -->  V4L2_CID_BRIGHTNESS\n", __FUNCTION__);
	...
	case V4L2_CID_CONTRAST:
		printk("DSG: %s -->  V4L2_CID_CONTRAST\n", __FUNCTION__);
		if (ctrl->val < 0 || ctrl->val > 255) {
	...
	case V4L2_CID_SATURATION:
		printk("DSG: %s V4L2_CID_SATURATION\n", __FUNCTION__);	
		if (ctrl->val < 0 || ctrl->val > 255) {
	...
	case V4L2_CID_AUTOGAIN:
		printk("DSG: %s -->  V4L2_CID_AUTOGAIN\n", __FUNCTION__);
		if (value == 1)
			value = 0x0F;
		else if (value == 0)
			value = 0x0C;
	...
	case V4L2_CID_TEST_PATTERN:
		printk("DSG: %s V4L2_CID_TEST_PATTERN\n", __FUNCTION__);	
		switch (ctrl->val) {
		case 0:
			break;
		}
