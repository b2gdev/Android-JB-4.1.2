...
#define AUTO_FLASH_GAIN_THREHOLD	96
...
/* OV5640 register set to configure back VGA preview, use with ov5640_new_reg_list */
static struct ov5640_reg ov5640_back_to_vga[] = {
	{0x3820, 0x41},
	{0x3821, 0x07},
	{0x3814, 0x31},
	{0x3815, 0x31},
...
/* OV5640 register set for 5M(2592x1944) capture, use with ov5640_new_reg_list */
static struct ov5640_reg ov5640_5m_cap[] = {
	{0x3820, 0x40},
	{0x3821, 0x06},
	{0x3814, 0x11},
	{0x3815, 0x11},
...
/* OV5640 new default register values */
static struct ov5640_reg ov5640_new_reg_list[] = {					//VGA streaming setup
	{0x3103, 0x11},
	{0x3008, 0x82},
	{0x3008, 0x42},
	{0x3103, 0x03},
...
static struct ov5640_reg ov5640_vga_reg[] = {							//Do not use for new_reg_list
	{0x3035, 0x11},
	{0x3036, 0x69},
	{0x3C07, 0x07},
	{0x3820, 0x41},
...
static struct ov5640_reg ov5640_5m_reg[] = {							//Do not use for new_reg_list
	{0x3035, 0x21},
	{0x3036, 0x54},
	{0x3C07, 0x07},
	{0x3820, 0x40},
...
static struct ov5640_reg ov5640_reg_list[] = {							//Do not use for new_reg_list
	{0x3103, 0x11},
	{0x3008, 0x82},
	{0x3008, 0x42},
	{0x3103, 0x03},
...
static struct ov5640_reg ov5640_af_reg[] = {
	{0x3000,0x20},		//DSG: System Reset 00 : (5: Reset MCU)
	{0x8000,0x02},
	{0x8001,0x0f},
	{0x8002,0xd6},
	{0x8003,0x02},
	...
	{0x3028,0x00},
	{0x3029,0x7F},
	{0x3000,0x00},		//DSG: System Reset 00 : Clear all reset bits
	{0x3004, 0xFF},		//DSG: Clock Enable 00
	{0, 0},
};
...
//DSG: Add set reg to support new stream implementation
static int ov5640_set_reg(struct i2c_client *client, unsigned short reg,
			     unsigned char val)
{
	int reg_value = 0;
...
//DSG: Add clear reg to support new stream implementation
 ...
 */
static int ov5640_get_light_frequency(struct v4l2_subdev *subdev)
{
 ...
 */
static int ov5640_get_sysclk(struct v4l2_subdev *subdev)
{
	// calculate sysclk
	int XVCLK = 2400;
	int temp1, temp2;
...
static int ov5640_new_capture(struct v4l2_subdev *subdev)
{
...
#if 0
	if (is_autoflash == 1) {
		ov5640_i2c_read_byte(client, 0x350b, &tmp);
	...
	if(1){ //is_awb_enabled){
		...
		if(average <= 5){ //AVG_LUM_THRESH){
			tmpreg = ov5640_read_reg(client, 0x3406);
			tmpreg = tmpreg | 0x01;
			ov5640_write_reg(client, 0x3406, tmpreg);

			ov5640_write_reg(client, 0x3400, 0x07);
			ov5640_write_reg(client, 0x3401, 0x30);
			ov5640_write_reg(client, 0x3402, 0x04);
			ov5640_write_reg(client, 0x3403, 0x00);
			ov5640_write_reg(client, 0x3404, 0x05);
			ov5640_write_reg(client, 0x3405, 0x50);

			if(1){ //is_effect_auto){
				ov5640_write_reg(client, 0x5583, 0x28);
				ov5640_write_reg(client, 0x5584, 0x28);
				tmpreg = ov5640_read_reg(client, 0x5588);
				tmpreg = tmpreg | 0x40;
				ov5640_write_reg(client, 0x5588, tmpreg);
			}
		}
	}
	...
	capture_VTS += tmpreg;
	capture_sysclk = ((uint) (ov5640_get_sysclk(subdev)));

	// calculate capture banding filter
	light_frequency = ov5640_get_light_frequency(subdev);
	...
	if(preview_shutter < PREVIEW_SHUTTER_MAX){
		...
		if(((preview_shutter == PREVIEW_SHUTTER_MAX) && (preview_gain16 > AUTO_FLASH_GAIN_THREHOLD) && (current_flasher_mode == V4L2_OV5640_FLASHER_AUTO)) || (current_flasher_mode == V4L2_OV5640_FLASHER_ON)){					//Flasher Adjustments
			capture_gain16_shutter = (capture_gain16_shutter * 4) / 5;			//Re-calibrate exposure to 80%
			fireflash = 1;
		}else{
			fireflash = 0;
		}
...
#if 0
static int ov5640_old_capture(struct v4l2_subdev *subdev)
{
	int ret = 0;
	u8 tmpreg = 0;
	uint prevGain = 0, capGain=0;
	uint prevFrame= 1500, capFrame = 500;
	uint prevExp = 0, capExp = 0;
	uint prevMaxlines = 980, capMaxlines = 1964;
	uint capExpGain = 0;
	uint capBandFilt = 0;
	uint lines_10ms;
	uint iCapGain;
	...
	if (ret) {
		debug_print("ov5640_new_reg_list failed\n");
		goto out;
	}
	debug_print("ov5640_new_reg_list written\n");	
	debug_print("Writing AEC/AGC manual\n");
	ret = ov5640_write_reg(client, 0x3503, 0x07);		//Manual AEC/AGC
	if (ret) { goto out; }
	
	debug_print("Reading preview register values\n");
	tmpreg = ov5640_read_reg(client, 0x3500);    		//Exposure
	prevExp = (((uint) (tmpreg & 0x0f)) << 12);
	tmpreg = ov5640_read_reg(client, 0x3501);
	prevExp += (((uint) tmpreg) << 4);
	tmpreg = ov5640_read_reg(client, 0x3502);
	prevExp += (tmpreg >> 4);
	tmpreg = ov5640_read_reg(client, 0x350a);			//Gain
	prevGain = (((uint) (tmpreg & 0x03)) << 8);
	tmpreg = ov5640_read_reg(client, 0x350b);
	prevGain += tmpreg;
	/*tmpreg = ov5640_read_reg(client, 0x350c);    		//Maxlines
	prevMaxlines = (((uint) tmpreg) << 8);
	tmpreg = ov5640_read_reg(client, 0x350d);
	prevMaxlines += tmpreg;*/
	lines_10ms = capFrame*capMaxlines/10000*13/12;
	debug_print("Preview Exposure %d, Gain %d, Maxlines %d\n",prevExp,prevGain,prevMaxlines);
	...
	debug_print("ov5640_5m_cap written\n");
	
	/*tmpreg = ov5640_read_reg(client, 0x350c);    		//Maxlines
	capMaxlines = (((uint) tmpreg) << 8);
	tmpreg = ov5640_read_reg(client, 0x350d);
	capMaxlines += tmpreg;*/
	debug_print("Capture Maxlines %d\n",capMaxlines);
	
	capExp = prevExp * (capFrame/prevFrame) * (capMaxlines/prevMaxlines) * (6/5);
	
	iCapGain = prevGain;
	capBandFilt = capFrame * capMaxlines / 100;
	
	capExpGain = capExp * iCapGain;
	
	if(capExpGain < (capMaxlines * 16)){
		capExp = capExpGain / 16;
		if(capExp > lines_10ms){
			capExp /= lines_10ms;
			capExp *= lines_10ms;
		}else{}
	}else{
	...
	iCapGain= (capExpGain*2/capExp + 1)/2;
	capGain=iCapGain;
	debug_print("Capture Exposure %d, Gain %d\n",capExp,capGain);
		
	tmpreg = (u8) ((capExp >> 12) & 0x0f);				//Exposure
	ret = ov5640_write_reg(client, 0x3500, tmpreg);	
	if (ret) { goto out; }
	tmpreg = (u8) ((capExp >> 4) & 0xff);
	ret = ov5640_write_reg(client, 0x3501, tmpreg);	
	if (ret) { goto out; }
	tmpreg = (u8) ((capExp & 0x0f) << 4);
	ret = ov5640_write_reg(client, 0x3502, tmpreg);	
	if (ret) { goto out; }
	tmpreg = (u8) ((capGain >> 8) & 0x03);				//Gain
	ret = ov5640_write_reg(client, 0x350a, tmpreg);	
	if (ret) { goto out; }
	tmpreg = (u8) (capGain & 0xff);
	ret = ov5640_write_reg(client, 0x350b, tmpreg);	
	if (ret) { goto out; }
	debug_print("Capture register values written\n");
...
static int ov5640_af_def_config(struct v4l2_subdev *subdev)
{
	...
	if (ret) {
		debug_print("DSG: %s() write ov5640_af_reg failed\n", __func__);
		v4l_err(client, "Failed to load af firmware\n");
	}
	else
	{
		debug_print("DSG: %s() write ov5640_af_reg completed\n", __func__);
		// wait until the status is idle
		while (ov5640_read_reg(client, OV5640_AF_REG_FW_STAT) != OV5640_AF_FW_IDLE) mdelay(1);
	}
...
static int ov5640_config_timing(struct v4l2_subdev *sd)
{
	...
	debug_print("DSG: frame size is %d\n", i);
	ret = ov5640_write_reg(client,
			0x3800,	//DSG: Timing HS (0x00) X address start high byte
			(timing_cfg[i].x_addr_start & 0xFF00) >> 8);
	if (ret)
		return ret;

	ret = ov5640_write_reg(client,
			0x3801,	//DSG: Timing HS (0x00) X address start low byte
			timing_cfg[i].x_addr_start & 0xFF);
	if (ret)
		return ret;

	ret = ov5640_write_reg(client,
			0x3802,	//DSG: Timing VS (0x00) Y address start high byte
			(timing_cfg[i].y_addr_start & 0xFF00) >> 8);
	if (ret)
		return ret;

	ret = ov5640_write_reg(client,
			0x3803,	//DSG: Timing VS (0x00) Y address start low byte
			timing_cfg[i].y_addr_start & 0xFF);
	if (ret)
		return ret;

	ret = ov5640_write_reg(client,
			0x3804,	//DSG: Timing HW (0x0A) X address end high byte
			(timing_cfg[i].x_addr_end & 0xFF00) >> 8);
	if (ret)
		return ret;

	ret = ov5640_write_reg(client,
			0x3805,	//DSG: Timing HW (0x3F) X address end low byte
			timing_cfg[i].x_addr_end & 0xFF);
	if (ret)
		return ret;

	ret = ov5640_write_reg(client,
			0x3806,	//DSG: Timing VH (0x07) Y address end high byte
			(timing_cfg[i].y_addr_end & 0xFF00) >> 8);
	if (ret)
		return ret;

	ret = ov5640_write_reg(client,
			0x3807,	//DSG: Timing VH (0x9F) Y address end low byte
			timing_cfg[i].y_addr_end & 0xFF);
	if (ret)
		return ret;

	ret = ov5640_write_reg(client,
			0x3808,	//DSG: Timing DVPHO (0x0A) DVP output horizontal width high byte
			(timing_cfg[i].h_output_size & 0xFF00) >> 8);
	if (ret)
		return ret;

	ret = ov5640_write_reg(client,
			0x3809,	//DSG: Timing DVPHO (0x20) DVP output horizontal width low byte
			timing_cfg[i].h_output_size & 0xFF);
	if (ret)
		return ret;

	ret = ov5640_write_reg(client,
			0x380A,	//DSG: Timing DVPVO (0x07) DVP output vertical height high byte
			(timing_cfg[i].v_output_size & 0xFF00) >> 8);
	if (ret)
		return ret;

	ret = ov5640_write_reg(client,
			0x380B,	//DSG: Timing DVPVO (0x98) DVP output vertical height low byte
			timing_cfg[i].v_output_size & 0xFF);
	if (ret)
		return ret;

	ret = ov5640_write_reg(client,
			0x380C,	//DSG: TIMING HTS (0x0B) Total horizontal size high byte
			(timing_cfg[i].h_total_size & 0xFF00) >> 8);
	if (ret)
		return ret;

	ret = ov5640_write_reg(client,
			0x380D,	//DSG: TIMING HTS (0x1C) Total horizontal size low byte
			timing_cfg[i].h_total_size & 0xFF);
	if (ret)
		return ret;

	ret = ov5640_write_reg(client,
			0x380E,	//DSG: TIMING VTS (0x07) Total vertical size high byte
			(timing_cfg[i].v_total_size & 0xFF00) >> 8);
	if (ret)
		return ret;

	ret = ov5640_write_reg(client,
			0x380F,	//DSG: TIMING VTS (0xB0) Total vertical size low byte
			timing_cfg[i].v_total_size & 0xFF);
	if (ret)
		return ret;

	ret = ov5640_write_reg(client,
			0x3810,	//DSG: TIMING HOFFSET (0x00) ISP horizontal offset high byte
			(timing_cfg[i].isp_h_offset & 0xFF00) >> 8);
	if (ret)
		return ret;

	ret = ov5640_write_reg(client,
			0x3811,	//DSG: TIMING HOFFSET (0x10) ISP horizontal offset low byte
			timing_cfg[i].isp_h_offset & 0xFF);
	if (ret)
		return ret;

	ret = ov5640_write_reg(client,
			0x3812,	//DSG: TIMING VOFFSET (0x00) ISP vertical offset high byte
			(timing_cfg[i].isp_v_offset & 0xFF00) >> 8);
	if (ret)
		return ret;

	ret = ov5640_write_reg(client,
			0x3813,	//DSG: TIMING VOFFSET (0x04) ISP vertical offset low byte
			timing_cfg[i].isp_v_offset & 0xFF);
	if (ret)
		return ret;

	ret = ov5640_write_reg(client,
			0x3814,	//DSG: TIMING X INC (0x11) Horizontal odd / even sub sample increment
	...
	ret = ov5640_write_reg(client,
			0x3815,	//DSG: TIMING Y INC (0x11) Vertical odd / even sub sample increment
...
static int ov5640_s_stream(struct v4l2_subdev *subdev, int streaming)
{
	...
	if (streaming) {
		unsigned char fmtreg = 0, fmtmuxreg = 0;
		int i;

		switch ((u32)ov5640->format.code) {
		case V4L2_MBUS_FMT_UYVY8_1X16:
			debug_print("DSG: Format code is V4L2_MBUS_FMT_UYVY8_1X16\n");
			fmtreg = 0x32;
			fmtmuxreg = 0;		//DSG: Format ISP YUV 422
			break;
		case V4L2_MBUS_FMT_YUYV8_1X16:
			debug_print("DSG: Format code is V4L2_MBUS_FMT_YUYV8_1X16\n");
			fmtreg = 0x30;
			fmtmuxreg = 0;		//DSG: Format ISP YUV 422
			break;
		case V4L2_MBUS_FMT_YUYV8_2X8:
			debug_print("DSG: Format code is V4L2_MBUS_FMT_YUYV8_2X8\n");
			fmtreg = 0x30;
			fmtmuxreg = 0;
			break;
		case V4L2_MBUS_FMT_UYVY8_2X8:
			debug_print("DSG: Format code is V4L2_MBUS_FMT_UYVY8_2X8\n");
			fmtreg = 0x32;
			fmtmuxreg = 0;
			break;
		default:
			/* This shouldn't happen */
			debug_print("DSG: Format code is unknown %08x\n", (u32)ov5640->format.code);
			ret = -EINVAL;
			return ret;
		}

		ret = ov5640_write_reg(client, 0x501F, fmtmuxreg);
		if (ret)
			return ret;
			
		ret = ov5640_write_reg(client, 0x4300, fmtreg);
		if (ret)
			return ret;

		ret = ov5640_config_timing(subdev);
		if (ret){
			debug_print("DSG: ov5640_config_timing failed\n");
			return ret;
		}
		...
		if ((i == OV5640_SIZE_QVGA) ||
		    (i == OV5640_SIZE_VGA) ||
		    (i == OV5640_SIZE_720P)) {
			//DSG: ret = ov5640_reg_set(client, 0x5001, 0x20);
			ret = ov5640_write_reg(client, 0x5001, 0xa3);
			debug_print("Scaling Enabled\n");
			//ret = ov5640_set_reg(client, 0x5001, 0x20);			//DSG: ISP Control Register - Enable scale
			if (ret)
				return ret;
			//ret = ov5640_write_reg(client, 0x3108,				//DSG: System root divider -
			//		(i == OV5640_SIZE_720P) ? 0x1 : 0);
		} else {
			//ret = ov5640_reg_clr(client, 0x5001, 0x20);
			ret = ov5640_clr_reg(client, 0x5001, 0x20);			//DSG: ISP Control Register - Disable scale
			if (ret)
				return ret;
			//ret = ov5640_write_reg(client, 0x3108, 0x2);		//DSG: System root divider -
		}

		//ret = ov5640_reg_clr(client, 0x3008, 0x40);
		ret = ov5640_clr_reg(client, 0x3008, 0x40);				//DSG: System Control 0 - Clear power down bit
		if (ret) {
			debug_print("DSG: clear power down bit failed\n");
			goto out;
		}
		debug_print("%s - New Stream started\n", __FUNCTION__);
		...
		if (ret)
			goto out;
		*/
		ret = ov5640_write_reg(client, 0x3008, tmpreg | 0x40);	//DSG: System Control 0 - Set power down but
		if (ret)
			goto out;
		debug_print("%s - New Stream stopped\n", __FUNCTION__);
	}
...
#else	//DSG: working stream code. Disabled to try new approach
static int mode = 0;
#include <linux/delay.h>
static int ov5640_s_stream(struct v4l2_subdev *subdev, int streaming)
{
	...
	if(streaming){
		debug_print("DSG: %s Start stream\n", __FUNCTION__);
...
//		{
			...
			ov5640_write_reg(client, 0x4745, 0x00);
			regValue = 0;
			regValue = ov5640_read_reg(client, 0x4745);
			debug_print("REG  0x4745 value %02X\n", regValue);

			regValue = (0x80 | (mode << 2) & 0xFF);
			ov5640_write_reg(client, 0x503d, regValue);
			mode++;
			if (mode == 4) {
				mode = 0;
			}
...
/*
			ov5640_write_reg(client, 0x4741, 0x05);
			regValue = 0;
			ov5640_reg_read(client, 0x4741, &regValue);
			debug_print("REG  0x4741 value %02X\n", regValue);
*/
 ...
 */
#define SIN_STEP 30
static const int ov5640_sin_table[] = { 0, 0x40, 0x6f, 0x80 };

static int ov5640_sine(int theta)
{
	int chs = 1;
	int sine;
	...
	if (theta <= 90)
		sine = ov5640_sin_table[theta/SIN_STEP];
	else {
		theta -= 90;
		sine = 0x80 - ov5640_sin_table[theta/SIN_STEP];
	}
	return sine*chs;
}

static int ov5640_cosine(int theta)
{
	theta = 90 - theta;
	if (theta > 180)
		theta -= 360;
		...
		theta += 360;
	return ov5640_sine(theta);
}

static int ov5640_strobe(struct i2c_client *client){
	if(fireflash){
...
static int ov5640_strobe_stop(struct i2c_client *client){
	if(fireflash){
...
static void ov5640_af_command(struct i2c_client *client,int af_command){
	...
	if (ret) {
		...
		debug_print("ENtered loop\n");
		while (ov5640_read_reg(client, OV5640_AF_REG_ACK) != 0x00){
			mdelay(1);
		}
...
static int ov5640_s_ctrl(struct v4l2_ctrl *ctrl)
{
	...
	if (ctrl == NULL) {
		debug_print("DSG: %s ctrl == NULL\n", __FUNCTION__);	
		return err;
	}

	value = (__s32) ctrl->val;
	debug_print("DSG: %s --> %08X\n", __FUNCTION__, ctrl->id);

	switch (ctrl->id) {
#if 0
	case V4L2_CID_BRIGHTNESS:
		debug_print("DSG: %s -->  V4L2_CID_BRIGHTNESS\n", __FUNCTION__);
		if (ctrl->val < 0 || ctrl->val > 255) {
			v4l_err(client, "invalid brightness setting %d\n",
					ctrl->val);
			return -ERANGE;
		}
		err = ov5640_write_reg(client, REG_BRIGHTNESS, value);
		if (err)
			return err;

		ov5640_reg_list[REG_BRIGHTNESS].val = value;
		break;
	case V4L2_CID_CONTRAST:
		debug_print("DSG: %s -->  V4L2_CID_CONTRAST\n", __FUNCTION__);
		if (ctrl->val < 0 || ctrl->val > 255) {
			v4l_err(client, "invalid contrast setting %d\n",
					ctrl->val);
			return -ERANGE;
		}
		err = ov5640_write_reg(client, REG_CONTRAST, value);
		if (err)
			return err;

		ov5640_reg_list[REG_CONTRAST].val = value;
		break;

	case V4L2_CID_SATURATION:
		debug_print("DSG: %s V4L2_CID_SATURATION\n", __FUNCTION__);	
		if (ctrl->val < 0 || ctrl->val > 255) {
		...
		err = ov5640_write_reg(client, 0x5001, (__u8)ov5640_read_reg(client, 0x5001) | 0x80);	// SDE En
		err = ov5640_write_reg(client, 0x5583, (__u8)value);	// saturation gain in U
		err = ov5640_write_reg(client, 0x5584, (__u8)value);	// saturation gain in V
		err = ov5640_write_reg(client, 0x5580, (__u8)ov5640_read_reg(client, 0x5580) | 0x02);	// saturation enable
		err = ov5640_write_reg(client, 0x5588, (__u8)ov5640_read_reg(client, 0x5588) | 0x40);
		if (err)
			return err;
		break;
	case V4L2_CID_HUE:
		debug_print("DSG: %s V4L2_CID_HUE\n", __FUNCTION__);	
		if (ctrl->val < -180 || ctrl->val > 180) {
		...
		err = ov5640_write_reg(client, 0x5001, (__u8)ov5640_read_reg(client, 0x5001) | 0x80);	// SDE En
		err = ov5640_write_reg(client, 0x5580, (__u8)ov5640_read_reg(client, 0x5580) | 0x01);	// hue enable
		err = ov5640_write_reg(client, 0x5583, (__u8)ov5640_cosine(value));	// fixed U
		err = ov5640_write_reg(client, 0x5584, (__u8)ov5640_sine(value));	// fixed V
		err = ov5640_write_reg(client, 0x5588, ((__u8)ov5640_read_reg(client, 0x5588) & ~0x33) | ((value<0)?((value<-90)?0x32:0x02):((value<90)?0x01:0x31)));
		if (err)
			return err;
		break;

	case V4L2_CID_AUTOGAIN:
		debug_print("DSG: %s -->  V4L2_CID_AUTOGAIN\n", __FUNCTION__);
		if (value == 1)
			value = 0x0F;
		else if (value == 0)
			value = 0x0C;
		...
		err = ov5640_write_reg(client, REG_AFE_GAIN_CTRL, value);
		if (err)
			return err;

		ov5640_reg_list[REG_AFE_GAIN_CTRL].val = value;
		break;

	case V4L2_CID_TEST_PATTERN:
		debug_print("DSG: %s V4L2_CID_TEST_PATTERN\n", __FUNCTION__);	
		switch (ctrl->val) {
		case 0:
			break;
		}
		/* FIXME Add proper Test pattern code here */
		return ov5640_write_reg(client, 0x503d, 0x80);
#endif
	...
	default:
		...
		debug_print("DSG: %s Default case\n", __FUNCTION__);
		return err;
	}
...
static const char * const v4l2_ov5640_flasher_mode_menu[] = {
	"auto",
	"on",
	"off",
	"torch",
