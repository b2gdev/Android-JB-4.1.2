...
static unsigned int poll_interval = 1; // 360;
					...
					union power_supply_propval *val)
{
	...
	case POWER_SUPPLY_PROP_STATUS:
		//printk("{RD} battery ptr:%p\n",di);
		ret = bq27x00_battery_status(di, val);
		//mutex_lock(&di->status_lock);
		//di->t_status = val->intval;
...
static int bq27x00_usb_get_prop(struct power_supply *psy,
			       enum power_supply_property psp,
			       union power_supply_propval *val)
{
	int ret = 0;
...
//{RD} BEGIN:
	di->usb.name = "bq27x00-usb",
	di->usb.type = POWER_SUPPLY_TYPE_USB;
	di->usb.properties = bq27x00_usb_props;
	di->usb.num_properties = ARRAY_SIZE(bq27x00_usb_props);
	di->usb.get_property = bq27x00_usb_get_prop;
	
	ret = power_supply_register(di->dev, &di->usb);
	
	if(ret)
		dev_err(di->dev, "Registering dummy usb power supply failed:%d\n",ret);
	else
		dev_info(di->dev, "Registered dummy usb power supply\n");
...
static const struct i2c_device_id bq27x00_id[] = {
	...
	{ "battery", BQ27000 },	/* bq27200 is same as bq27000, but with i2c */
	{ "bq27500", BQ27500 },
	{},
};
