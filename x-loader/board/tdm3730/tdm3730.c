...
#if 0
/*******************************************************
 * Routine: omap3_enable_dss
 * Description: Enable DSS and display splash
 ********************************************************/
#define TIMING(bp, fp, sw) (((bp) << 20) | ((fp) << 8) | (sw))
#define DSS_DISPC_BASE 0x48050440

unsigned char *fb_addr[800*480*4];

void omap3_enable_dss() {
        u32 l;
        unsigned char *fb_addr;
        int i;
        
        __raw_writel(TIMING(46, 210, 1), DSS_DISPC_BASE + 0x64); /* h timing */
        __raw_writel(TIMING(23, 22, 1),  DSS_DISPC_BASE + 0x68); /* v timing */
        __raw_writel(0x7028, DSS_DISPC_BASE + 0x6c); /* pol freq */
        __raw_writel(0x00010003, DSS_DISPC_BASE + 0x70); /* divisor */

        /* 800 x 480 resolution */
        __raw_writel(0x01df031f, DSS_DISPC_BASE + 0x7c); /* LCD size */
        __raw_writel(0x01e00320, DSS_DISPC_BASE + 0x8c); /* gfx size */

        /* Load only frame data every frame (not gamma) */
        sr32(DSS_DISPC_BASE+0x44, 1, 2, 0x02); 

        /* Set 32 bit rgba pixels */
        sr32(DSS_DISPC_BASE+0xa0, 4, 1, 0x0e); /* 0x0c = bgra, 0x0d = rgba, 0x0e = rgbx32 */
        sr32(DSS_DISPC_BASE+0x40, 8, 2, 0x03); /* 24 wires */
        
        /* frame buffer adress */
        if (fb_addr != NULL) {
                __raw_writel(DSS_DISPC_BASE+0x80, (u32)fb_addr);
                for (i=0; i<800*480*4; i++) fb_addr[i] = i&255;
        } else
                printf("Malloc failed\n");

        /* enable lcd */
        sr32(DSS_DISPC_BASE+0x40, 3, 1, 0x01); /* set active display (TFT) */
        sr32(DSS_DISPC_BASE+0x40, 5, 1, 0x01); /* go lcd */
        sr32(DSS_DISPC_BASE+0x40, 0, 1, 0x01); /* enable lcd */        
}
#endif
