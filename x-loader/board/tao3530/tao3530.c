...
void config_3430sdram_ddr(void)
{
	...
	if(tao3530_revision() < 3) /* 256MB / Bank */
	{
...
int tao3530_revision(void)
{
        ...
        omap_request_gpio(1);
        omap_set_gpio_direction(65,1);
        omap_set_gpio_direction(1,1);

        rev = omap_get_gpio_datain(65) << 1 |
                omap_get_gpio_datain(1);
...
/*******************************************************
 * Routine: misc_init_r
 * Description: Print revision information
 ********************************************************/
int misc_init_r(void)
{
        int rev;
        rev = tao3530_revision();
...
#define MUX_DEFAULT()\
	...
	MUX_VAL(CP(SYS_CLKREQ),     (IEN  | PTU | EN  | M4)) /*GPIO_1*/\
