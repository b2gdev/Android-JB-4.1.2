...
int misc_init_r(void)
{
	...
	*(ulong *)(CONTROL_PROG_IO1) &= ~(PRG_I2C2_PULLUPRESX);
	...
	case REVISION_C4:
#ifdef SYSCHECK
		printf("TCBIN Rev C - syscheck 1.6\n"); /* {PS} : printf("Beagle Rev C4\n"); */
#else
		printf("TCBIN Rev C\n"); /* {PS} : printf("Beagle Rev C4\n"); */
#endif		
		/* {PS} : MUX_BEAGLE_C(); */
	...
	/* {PS} : Set output value */
	omap_set_gpio_dataout(11, 0);		/* {RD} : 3GM_SEC_PWR_EN	- LOW	- Turn off 3G modem via secondary enable pin */	
	omap_set_gpio_dataout(16, 1);		/* {PS} : 3GM_PWR_nEN		- HIGH	- Turn off 3G modem power supply */
	udelay(1000000);
