...
1)
Command: sudo ./mkmmc-android <device> <MLO> <u-boot.img> <uImage> <uEnv.txt> <rootfs tar.bz2 > <Media_Clips> <START_HERE Location>
Example: sudo ./mkmmc-android /dev/sdc MLO u-boot.img uImage uEnv.txt rootfs.tar.bz2 Media_Clips
Details: In this case, the script will take Boot Images (MLO,u-boot.img, uImage and uEnv.txt) Root Filesystem tarball, Media Clips and START_HERE folder as arguements.
...
Example: sudo ./mkmmc-android /dev/sdc MLO u-boot.img uImage uEnv.txt rootfs.tar.bz2
Details: In this case, the script will take Boot Images (MLO,u-boot.img, uImage and uEnv.txt) and Root Filesystem tarball as arguements.
         The script will then put the boot images on boot partition, extract the android rootfs-rootfs_*.tar.bz2 to rootfs partition.
